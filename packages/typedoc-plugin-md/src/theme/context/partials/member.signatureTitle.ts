import type { Reflection, SignatureReflection } from 'typedoc'
...
export function signatureTitle(
  ...
  if (!['__call', '__type'].includes(model.name)) {
    ...
    if (
      [ReflectionKind.GetSignature, ReflectionKind.SetSignature].includes(
        anchorModel.kind,
      )
    ) {
    ...
    const anchor =
      anchorModel &&
      this.router.hasUrl(anchorModel) &&
      this.router.getAnchor(anchorModel)
